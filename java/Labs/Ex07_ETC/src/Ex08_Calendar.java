...
public class Ex08_Calendar {
	...
	public static void main(String[] args) {
		...
		System.out.println(cal); //toString 재정의(날짜 정보 문자열 나열)
		...
		//시 분 초
		System.out.println("몇 시 : " + cal.get(Calendar.HOUR));
		System.out.println("몇 분 : " + cal.get(Calendar.MINUTE));
		System.out.println("몇 초 : " + cal.get(Calendar.SECOND));
		//오전 오후(AM PM)
		System.out.println("오전 호후 : " + cal.get(Calendar.AM_PM));
		//오전 = 0 / 오후 = 1
		
		//웹 사이트(교육 시스템)
		//150본 (페이지) >> 120페이지 날짜 정보를 가지고 있다 (서버쪽 시간)
		//2018년 02월 12일  >> 변경  >> 2018-02-12
		//날짜 정보를 한번만 수정해서 전체 반영  : 클래스 >> 함수 (static) >> 날짜
		String date = Edu_Date.DateString(Calendar.getInstance());
		System.out.println(date);
		String date2 = Edu_Date.DateString(Calendar.getInstance(), "::");
		System.out.println(date2);
		String date3 = Edu_Date.monthFormet_DateString(Calendar.getInstance());
